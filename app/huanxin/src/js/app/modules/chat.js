...
function _bindEvents(){
	...
	utils.on(doms.textInput, "compositionstart", function(){
		isMessageChineseEnd = false;
	});
	utils.on(doms.textInput, "compositionend", function(){
		isMessageChineseEnd = true;
		handleSendBtn()
	});
	function handleSendBtn(){
		...
		utils.toggleClass(
			doms.sendBtn,
			"disabled",!isMessageChineseEnd|| !isMessageChannelReady || isEmpty
		);
	...
	if(utils.isQQBrowser && utils.isAndroid){
		//doms.imgInput.setAttribute("capture","camera");

...
function handleMsgData(){
	...
	profile.defaultCustomer = config.staticPath + "/img/customer.png";

	if(profile.CustomerImg == null){
		if(!config.visitor.photo){
			profile.CustomerImg = profile.defaultCustomer;
		}else {
			var img = new Image();
			img.src =config.visitor.photo;
			if (img.complete) {
				Wcallback()
			}else {
				img.onload = Wcallback;
			}
			document.addEventListener("error", imgError, true);
			function Wcallback(){
				profile.CustomerImg = config.visitor.photo;
				document.removeEventListener("error",imgError,true)
			}
			function imgError(e){
				var elem = e.target;
				if(elem.tagName.toLowerCase() === 'img'){
					elem.src = profile.defaultCustomer;
					profile.CustomerImg = profile.defaultCustomer;
				}
			}
		}
